...
def fetch_and_augment_stock_data(
        tickers=[
            'AAPL',
            'MSFT',
            'GOOGL',
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        for ticker in tickers:
            ...
            for date, row in ticker_data.iterrows():
                base_price = row['Open']
                # Sometimes High/Low can be Series or scalar depending on
                # yfinance structure
                ...
                for _ in range(records_per_day):
                    ...
                    f.write(
                        f"{
                            random_time.isoformat()},{ticker},{
                            price:.2f},{tx_type},{volume},{
                            total_value:.2f}\n")
                    total_generated += 1
                    ...
                    if total_generated % 500_000 == 0:
                        print(
                            f"  Progress: {total_generated:,} / {target_records:,}")
                if total_generated >= target_records:
                    break
            if total_generated >= target_records:
                break
