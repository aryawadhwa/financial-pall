...
def process_chunk(file_path: str, start: int, end: int, worker_id: int) -> StockSummary:
    ...
    # ⚡ Bolt Optimization: Using .split('\n') avoids the overhead of Universal Newlines
    # associated with .splitlines(), providing a slight parsing speedup.
    # Note: Python's float() safely ignores trailing '\r', making this safe for Windows files.
